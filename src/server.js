...
function criarServidor() {
  ...
  app.post('/imprimir', async (req, res) => {
    const { pedido, nomeLoja, forte, fonteGrande, cnpj, larguraPapel: larguraReq } = req.body
    ...
    try {
      ...
      await imprimir(pedido, nomeLoja || '', cfg.porta, largura, modoVia, !!forte, !!fonteGrande, cnpj || '')
      res.json({ ok: true })
    } catch (e) {
  ...
  app.post('/testar', async (req, res) => {
    ...
    try {
      const { forte, fonteGrande, cnpj, larguraPapel: larguraReq } = req.body || {}
      const largura = Number(larguraReq) === 80 || Number(larguraReq) === 58 ? Number(larguraReq) : (cfg.larguraPapel || 58)
      await testar(cfg.porta, largura, !!forte, !!fonteGrande, cnpj || '')
      res.json({ ok: true })
    } catch (e) {
